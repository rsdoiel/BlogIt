...
function Build-It {
    ...
    $osMap = @{
	    ...
	    "piOS" = "unknown-linux-gnu"
    }
    if ($archMap.ContainsKey($TargetOS)) {
            $TargetOS = $archMap[$TargetOS]
    }
    ...
    if ([string]::IsNullOrEmpty($TargetArch)) {
        ...
        if ($archMap.ContainsKey($arch)) {
            ...
            $uname = uname -m
            if ($uname -eq "x86_64") { $TargetArch = "x86_64" }
            elseif ($uname -eq "aarch64") { $TargetArch = "aarch64" }
            else { throw "Unsupported host architecture: $arch" }
        }
    }

    # Validate supported OS and architecture
    $supportedOS = @("windows-msvc", "darwin", "unknown-linux-gnu")
    ...
    if ($TargetArch -notin $supportedArch) {
        Write-Error "Unsupported architecture: $TargetArch. Use x86_64 or aarch64."
        exit 1
    }
    ...
    # Run the deno compile command
    deno compile --target "$TargetArg-$TargetOS" --allow-read --allow-write --output "$OutPath" "$SourcePath" 
    if (-not $?) {
